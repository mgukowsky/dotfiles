...
local function on_attach(_, bufnr)
  ...
  -- Movement mappings
  local next_diag_repeat, prev_diag_repeat = ts_repeat_move.make_repeatable_move_pair(function()
  ...
  lspconfig = {
    on_attach = function(client, bufnr)
      ...
      wk.register({
        ["<leader>"] = {
          l = {
            x = { function() require("telescope").extensions.yaml_schema.yaml_schema() end, "Select YAML schema" },
          },
        },
      }, {
        mode = "n",
  ...
  wk.register({
    ["<leader>"] = {
      d = {
        q = {
          d = {
            function() util.run_if_compile_commands(cpp_util.dbg_select) end,
            "Select program to debug"
          },
          D = {
            function() util.run_if_compile_commands(cpp_util.run_gtest_at_cursor) end,
            "Debug gtest at cursor"
          },
        },
      },
    },
    ...
    ["<F17>"] = {
      function()
        util.run_if_compile_commands(
          function() cpp_util.dbg_select({ enter_args = true }) end
        )
      end,
  ...
  wk.register({
    ["<leader>"] = {
      d = {
        q = {
          d = {
            function() rlsp("debuggables") end,
            "Select program to debug",
          },
          D = {
            function() rlsp("debug") end,
            "Debug target at cursor",
          },
        },
      },
      l = {
        a = {
          function() rlsp("codeAction") end,
          "Code action (Rust)",
        },
        r = {
          name = "Rust functions",
          c = {
            function() rlsp("openCargo") end,
            "Cargo.toml",
          },
          e = {
            function() rlsp("explainError") end,
            "Explain error",
          },
          E = {
            function() rlsp("renderDiagnostic") end,
            "Render diagnostic",
          },
          -- Only useful if checkOnSave for rust-analyzer is false
          f = {
            function() rlsp("flyCheck") end,
            "Fly check (cargo/clippy)",
          },
          g = {
            function() rlsp("crateGraph") end,
            "View crate DAG",
          },
          m = {
            function() rlsp("expandMacro") end,
            "Expand macro",
          },
          o = {
            function() rlsp("openDocs") end,
            "Open docs",
          },
          p = {
            function() rlsp("parentModule") end,
            "Parent module",
          },
          r = {
            function() rlsp("runnables") end,
            "Runnables select",
          },
          R = {
            function() rlsp("run") end,
            "Run target at cursor",
          },
          s = {
            function() rlsp("syntaxTree") end,
            "View syntax tree",
          },
          t = {
            function() rlsp("testables") end,
            "Testables select",
          },
          x = {
            name = "eXtended functionality",
            a = {
              function() vim.cmd.RustEmitAsm() end,
              "View ASM",
            },
            h = {
              function() rlsp({ "view", "hir" }) end,
              "View HIR",
            },
            i = {
              function() vim.cmd.RustEmitIr() end,
              "View LLVM IR",
            },
            l = {
              function() rlsp("logFile") end,
              "rust-analyzer logs",
            },
            m = {
              function() rlsp({ "view", "mir" }) end,
              "View MIR",
            },
          },
        },
    ...
    ["+"] = {
      function() rlsp({ "hover", "actions" }) end,
      "Show hover (press twice to focus)",
    },
    ["<F5>"] = {
      function() rlsp({ "debuggables", bang = true }) end,
      "Run last debuggable",
    },

    -- Ctrl+F5; same as Visual Studio mapping
    ["<F29>"] = {
      function() rlsp({ "runnables", bang = true }) end,
      "Run last runnable",
    },
  }, {
    mode = "n",
