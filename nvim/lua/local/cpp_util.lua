...
local function get_cursor_gtest_info()
	...
	while true do
		if
			vim.treesitter.get_node_text(current_node, 0):match("^TEST")
			and current_node:type() == "function_definition"
		then
...
function M.dbg_select(opts)
	...
	local promptstring = "Executable to debug:"
	if opts.enter_args == true then
		promptstring = "Executable to debug (w/ arguments):"
	end
	...
	vim.ui.select(executables, {
		...
		if choice ~= nil then
			...
			dap_util.run_dap_config(outdir .. "/" .. choice, choice, args)
		end
	end)
end
...
function M.run_gtest_at_cursor()
	...
	local ctest_json =
		require("dap.ext.vscode").json_decode(util.stdout_exec("cd " .. outdir .. " && ctest --show-only=json-v1"))
